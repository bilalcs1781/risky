    ...
    try {
      const fetchData = async () => {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/orderDetail?id=${OrderId}`
        );
        ...
        <div className="container my-5">
          ...
          <div className="row" id="orderDetail">
            <div className="col-12 font">
              <p>
                <b style={{}}>Name:</b> {order?.name1} {order?.name2}
              </p>
              {order?.email && (
                <p>
                  <b style={{}}>E-mail: </b>
              ...
              <p>
                <b style={{}}>Street & House Number: </b>
                {order?.street}
              </p>
              {order?.appartment && (
                <p>
                  <b style={{}}>Appartment: </b>
                  {order?.appartment}
                </p>
              )}
              {order?.postal && (
              ...
              <>
                <div className="cart_display_layout1">
                  {order?.orderItems?.map((item, index) => {
                    return (
                      <div
                        className="d-flex gap-4 my-3 border py-3"
                        style={{
                      ...
                      >
                        <div className="row">
                          <div className="col-4">
                            <a
                              href={`/product/${item.title.replace(
                                / /g,
                                "-"
                              )}/${item.productId}`}
                            >
                          ...
                          <div className="col-8">
                            <div className="w-100 px-2">
                              ...
                              <div className="py-2 d-flex justify-content-between align-items-center">
                                <p className="m-0 fs-6">Category</p>
                                <p className="m-0 fs-6">{item?.category}</p>
                              </div>
                              <hr className="m-0 p-0" />
                              <div className="py-2 d-flex justify-content-between align-items-center">
                                <p className="m-0 fs-6">Size</p>
                              ...
                              <div className="py-2 d-flex justify-content-between align-items-center">
                                <p className="m-0 fs-6">Price</p>
                                <p className="m-0 fs-6">&pound;{item?.price}</p>
                              </div>
                              <hr className="m-0 p-0" />

                              <div className="py-2 d-flex justify-content-between align-items-center">
                                <p className="m-0 fs-6">Quantity</p>
                                <p className="m-0  fs-6">{item?.quantity}</p>
                              </div>
                              <hr className="m-0 p-0" />

                              <div className="py-2 d-flex justify-content-between align-items-center">
                                <p className="m-0 fs-6">Total Price</p>
                                <p className="m-0 fs-6">
                        ...
                        {order?.orderItems ? (
                          order?.orderItems?.map((data, index) => (
                            <tr key={index}>
                              ...
                              <td>
                                <a
                                  href={`/product/${data.title.replace(
                                    / /g,
                                    "-"
                                  )}/${data.productId}`}
                                >
                              ...
                              <td className="text-center">
                                {data.size ? data.size : "Not Selected"}
                              </td>
                              <td className="text-center">
                                {data.color ? data.color : "Not Selected"}
                              </td>
                              {/* <td className='text-center'>{data?.subCategory ? data?.subCategory : "No subcategory"}</td> */}
                              <td className="text-center">
                                {`£${parseFloat(data?.price)?.toFixed()}`}.00
                              </td>
                              <td className="text-center">{`${parseInt(
                                data?.quantity
                              )}`}</td>
                              ...
                              <td className="text-center">
                                {`£${parseFloat(data?.total)?.toFixed()}`}.00
                              </td>
                            </tr>
                          ))
                        ) : (
          ...
          <div>
            {cu.role === "admin" && (
              <a href={`/admin-dashboard`}>
                <button className="button-submit px-5">Dashboard</button>
              </a>
            )}
            {cu.role !== "admin" && (
              <a href={`/`}>
